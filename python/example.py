...
# from example_analyze_eeg import *
...
# _TARGET_DEVICE_NAME = "BR-45566"

SRLOG.LOG_INFO("StarkSDK version:" + StarkSDK.get_sdk_version())
...
def serial_read_value():
    ...
    hex_data = ','.join(['{:02X}'.format(byte) for byte in receive_frame])
...
if __name__ == '__main__':
    print(serial_ports())
    print(serial_open(serial_port_name))
    StarkSDK.set_write_data_callback(serial_write_data)
    device = StarkDevice.create_serial_device(99, "BR-99")
    ...
    # setters
    device.set_serial_device_id(100)
    device.set_serialport_cfg(9600)
    device.set_max_current(1000)
    ...
    device.set_led_info(LedMode.blink, LedColor.rb)
